...
def scan_gmaps(apikey):
    ...
    if response.text.find("error_message") < 0:
        ...
        print url
        vulnerable_apis.append(
            "Query Autocomplete-Places 	|| $2.83 per 1000 requests")
    else:
        print "API key is not vulnerable for Text Search-Places API."
        print "Reason: " + response.json()["error_message"]
...
if len(sys.argv) > 1:
    if sys.argv[1] == "--api-key" or sys.argv[1] == "-a":
        if len(sys.argv) > 2:
            ...
            print "Either use --api-key as argument such \"python gmap_api_scanner.py --api-key KEY\" or directly run script as \"python gmap_api_scanner.py\" and supply API key via input."
    ...
    apikey = raw_input(
        "Please enter the Google Maps API key you wanted to test: ")
    scan_gmaps(apikey)
